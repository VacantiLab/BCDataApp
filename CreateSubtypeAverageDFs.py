def CreateSubtypeAverageDFs(df_p,df_m,subtypes):
    ...
    df_p_subtype_sd.loc[:,subtypes[0]] = np.std(df_p.iloc[:,0:9].T)
    df_p_subtype_sd.loc[:,subtypes[1]] = np.std(df_p.iloc[:,9:18].T)
    df_p_subtype_sd.loc[:,subtypes[2]] = np.std(df_p.iloc[:,18:27].T)
    df_p_subtype_sd.loc[:,subtypes[3]] = np.std(df_p.iloc[:,27:36].T)
    df_p_subtype_sd.loc[:,subtypes[4]] = np.std(df_p.iloc[:,36:45].T)
    ...
    df_m_subtype_sd.loc[:,subtypes[0]] = np.std(df_m.iloc[:,0:9].T)
    df_m_subtype_sd.loc[:,subtypes[1]] = np.std(df_m.iloc[:,9:18].T)
    df_m_subtype_sd.loc[:,subtypes[2]] = np.std(df_m.iloc[:,18:27].T)
    df_m_subtype_sd.loc[:,subtypes[3]] = np.std(df_m.iloc[:,27:36].T)
    df_m_subtype_sd.loc[:,subtypes[4]] = np.std(df_m.iloc[:,36:45].T)
